  ...
  getSubCategoryById: async (req, res) => {
    try {
      ...
      const subCategory = await subCategoryModel
        ...
        .populate({
          path: "products",
          model: "product", // Assure-toi que le modèle s'appelle bien "Product" (et non "product")
        });
  ...
  getProductBySubCategory: async (req, res) => {
    try {
      ...
      if (!subCategory) {
        return res.status(404).json({
          success: false,
          message: "product by subCategory  not found",
          data: null,
        });
      }
      res.status(200).json({
        success: true,
        message: "product by subCategory found",
